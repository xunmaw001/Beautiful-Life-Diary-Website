        ...
        <div class="ng-inmod-ini">
          <div class="ng-fault ng-container">
            <div class="ng-inmod-the ng-container ng-bg-fff">
              ...
              <span class="ng-inmod-the-cn" aos="fade-down">日记展示展示</span><span class="ng-inmod-the-en" aos="fade-up">DATA SHOW</span>
            </div>
          ...
          <fieldset class="search-container" style="text-align: center;">
            <form class="layui-form">
              <div class="layui-inline" style="margin-bottom: 10px;">
                <label class="layui-form-label">日记编号</label>
                <div class="layui-input-inline">
                  <input type="text" name="rijibianhao" id="rijibianhao" placeholder="日记编号" autocomplete="off" class="layui-input">
                </div>
              </div>   
              <div class="layui-inline" style="margin-bottom: 10px;">
                <label class="layui-form-label">推荐原因</label>
                <div class="layui-input-inline">
                  <input type="text" name="tuijianyuanyin" id="tuijianyuanyin" placeholder="推荐原因" autocomplete="off" class="layui-input">
                </div>
              </div>   
              <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
                ...
                <button v-if="isAuth('rijizhanshi','新增')" @click="jump('../rijizhanshi/add.jsp')" type="button" class="layui-btn btn-theme">
              ...
              <ul>
                <li v-for="(item,index) in dataList" v-bind:key="index">
                  <div class="ng-prslist-doc">
                    <div class="ng-prslist-img" @click="jump('../rijizhanshi/detail.jsp?id='+item.id)">
                    ...
                    <div class="ng-prslist-tit" @click="jump('../rijizhanshi/detail.jsp?id='+item.id)"><span>{{item.rijibianhao}}</span></div>
                    <div class="ng-prslist-tit" @click="jump('../rijizhanshi/detail.jsp?id='+item.id)"><span>{{item.rijibiaoti}}</span></div>
                    <div class="ng-prslist-tit" @click="jump('../rijizhanshi/detail.jsp?id='+item.id)"><span>{{item.rijineirong}}</span></div>
                    <div class="ng-prslist-tit" @click="jump('../rijizhanshi/detail.jsp?id='+item.id)"><span>{{item.xingming}}</span></div>
                    <div class="ng-prslist-tit" @click="jump('../rijizhanshi/detail.jsp?id='+item.id)"><span>{{item.tuijianyuanyin}}</span></div>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#rijibianhao').val()) {
          param['rijibianhao'] = jquery('#rijibianhao').val() ? '%' + jquery('#rijibianhao').val() + '%' : '';
        }  
        if (jquery('#tuijianyuanyin').val()) {
          param['tuijianyuanyin'] = jquery('#tuijianyuanyin').val() ? '%' + jquery('#tuijianyuanyin').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('rijizhanshi/list', 'get', param, function(res) {
